...
<head>
    ...
    <title>Admin — {{ config('app.name', 'AI Resume Builder') }}</title>
            ...
            <div class="flex h-16 items-center gap-3 px-4 sm:gap-4 sm:px-6 lg:px-8">
                ...
                <div class="panel-shell min-w-0 flex-1">
                    @isset($header)
                        {{ $header }}
                    @else
                        <h1 class="text-lg font-bold panel-title">Admin</h1>
                    @endisset
                </div>
                ...
                <div class="hidden items-center gap-3 sm:flex">
                    <span class="admin-badge bg-indigo-50 text-indigo-700 ring-1 ring-indigo-100 dark:bg-indigo-500/15 dark:text-indigo-300 dark:ring-indigo-500/30">Administrator</span>
                    <span class="text-xs text-slate-400 dark:text-slate-500">{{ now()->format('M j, Y') }}</span>
                </div>
            </div>
        </header>
